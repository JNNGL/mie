...
namespace mie {
    ...
    double Solver::computeExtinctionCrossSection(const Particle& particle, double wavelength) {
        return wavelength * wavelength / (2.0 * M_PI) * computeScatteringAmplitudes(particle, 1.0, wavelength).abReal;
    }

    double Solver::computeScatteringCrossSection(const Particle& particle, double wavelength) {
        double abNorm = computeScatteringAmplitudes(particle, 1.0, wavelength).abNorm;
        ...
        return abNorm * wavelength * wavelength * std::exp(-alpha) / (2.0 * M_PI * gamma * std::norm(particle.etaHost));
     }
